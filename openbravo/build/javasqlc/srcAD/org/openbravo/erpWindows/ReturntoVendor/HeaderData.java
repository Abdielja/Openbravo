 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE C_Order" +
      "        SET DocAction = (?) , RM_Pickfromreceipt = (?) , AD_Org_ID = (?) , C_DocTypeTarget_ID = (?) , DocumentNo = (?) , Generatetemplate = (?) , POReference = (?) , DateOrdered = TO_DATE(?) , C_BPartner_ID = (?) , C_BPartner_Location_ID = (?) , C_Return_Reason_ID = (?) , M_Warehouse_ID = (?) , DatePromised = TO_DATE(?) , RM_ReceiveMaterials = (?) , FIN_Paymentmethod_ID = (?) , RM_CreateInvoice = (?) , C_PaymentTerm_ID = (?) , M_PriceList_ID = (?) , DocStatus = (?) , GrandTotal = TO_NUMBER(?) , TotalLines = TO_NUMBER(?) , C_Currency_ID = (?) , AD_User_ID = (?) , C_Project_ID = (?) , Deliverynotes = (?) , Description = (?) , SalesRep_ID = (?) , BillTo_ID = (?) , C_Incoterms_ID = (?) , Incotermsdescription = (?) , IsDiscountPrinted = (?) , C_DocType_ID = (?) , FreightAmt = TO_NUMBER(?) , DeliveryViaRule = (?) , M_Shipper_ID = (?) , PriorityRule = (?) , ChargeAmt = TO_NUMBER(?) , C_Charge_ID = (?) , FreightCostRule = (?) , CopyFrom = (?) , CopyFromPO = (?) , RM_AddOrphanLine = (?) , C_Activity_ID = (?) , C_Campaign_ID = (?) , AD_OrgTrx_ID = (?) , User1_ID = (?) , User2_ID = (?) , EM_Obpos_Createdabsolute = TO_TIMESTAMP(?, ?) , Processing = (?) , DatePrinted = TO_DATE(?) , Processed = (?) , Posted = (?) , IsSOTrx = (?) , AD_Client_ID = (?) , IsInvoiced = (?) , DateAcct = TO_DATE(?) , DropShip_User_ID = (?) , PaymentRule = (?) , IsActive = (?) , IsSelfService = (?) , IsDelivered = (?) , IsPrinted = (?) , C_Order_ID = (?) , DeliveryRule = (?) , DropShip_BPartner_ID = (?) , IsTaxIncluded = (?) , DropShip_Location_ID = (?) , IsSelected = (?) , InvoiceRule = (?) , Delivery_Location_ID = (?) , updated = now(), updatedby = ? " +
      "        WHERE C_Order.C_Order_ID = ? " +
      "        AND C_Order.AD_Client_ID IN (";
    strSql = strSql + ((adUserClient==null || adUserClient.equals(""))?"":adUserClient);
    strSql = strSql + 
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        INSERT INTO C_Order " +
      "        (DocAction, RM_Pickfromreceipt, AD_Org_ID, C_DocTypeTarget_ID, DocumentNo, Generatetemplate, POReference, DateOrdered, C_BPartner_ID, C_BPartner_Location_ID, C_Return_Reason_ID, M_Warehouse_ID, DatePromised, RM_ReceiveMaterials, FIN_Paymentmethod_ID, RM_CreateInvoice, C_PaymentTerm_ID, M_PriceList_ID, DocStatus, GrandTotal, TotalLines, C_Currency_ID, AD_User_ID, C_Project_ID, Deliverynotes, Description, SalesRep_ID, BillTo_ID, C_Incoterms_ID, Incotermsdescription, IsDiscountPrinted, C_DocType_ID, FreightAmt, DeliveryViaRule, M_Shipper_ID, PriorityRule, ChargeAmt, C_Charge_ID, FreightCostRule, CopyFrom, CopyFromPO, RM_AddOrphanLine, C_Activity_ID, C_Campaign_ID, AD_OrgTrx_ID, User1_ID, User2_ID, EM_Obpos_Createdabsolute, Processing, DatePrinted, Processed, Posted, IsSOTrx, AD_Client_ID, IsInvoiced, DateAcct, DropShip_User_ID, PaymentRule, IsActive, IsSelfService, IsDelivered, IsPrinted, C_Order_ID, DeliveryRule, DropShip_BPartner_ID, IsTaxIncluded, DropShip_Location_ID, IsSelected, InvoiceRule, Delivery_Location_ID, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), (?), (?), (?), (?), (?), (?), TO_DATE(?), (?), (?), (?), (?), TO_DATE(?), (?), (?), (?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), TO_NUMBER(?), (?), (?), (?), TO_NUMBER(?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), TO_TIMESTAMP(?, ?), (?), TO_DATE(?), (?), (?), (?), (?), (?), TO_DATE(?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), now(), ?, now(), ?)";
