 ...
 */
  public static String selectAuxF94E1C2071A54A7CB8F4AFEA71974D43(ConnectionProvider connectionProvider, String AD_Client_ID, String AD_ORG_ID)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        select (case count(AD_Preference_id) when 0 then 'N' else 'Y' end ) from AD_Preference where property='OBPOS_remote.product' and to_char(value)='Y' and  ad_client_id = ? and AD_ISORGINCLUDED(?, AD_ORG_ID, AD_CLIENT_ID) <> -1";
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET AD_Org_ID = (?) , Name = (?) , Description = (?) , Isactive = (?) , Includeall_Product = (?) , Obretco_Productlist_ID = (?) , AD_Client_ID = (?) , updated = now(), updatedby = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Org_ID, Name, Description, Isactive, Includeall_Product, Obretco_Productlist_ID, AD_Client_ID, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), (?), (?), (?), (?), (?), (?), now(), ?, now(), ?)";
