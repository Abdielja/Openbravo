 ...
 */
  public static HeaderData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_Order.EM_Obpos_Applications_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table14.Name), ''))),'') ) END) AS EM_Obpos_Applications_IDR, " +
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE C_Order" +
      "        SET AD_Org_ID = (?) , C_DocTypeTarget_ID = (?) , C_Return_Reason_ID = (?) , DocumentNo = (?) , DateOrdered = TO_DATE(?) , C_BPartner_ID = (?) , C_BPartner_Location_ID = (?) , RM_PickFromShipment = (?) , M_PriceList_ID = (?) , RM_ReceiveMaterials = (?) , DatePromised = TO_DATE(?) , RM_CreateInvoice = (?) , FIN_Paymentmethod_ID = (?) , C_PaymentTerm_ID = (?) , M_Warehouse_ID = (?) , InvoiceRule = (?) , DocStatus = (?) , GrandTotal = TO_NUMBER(?) , TotalLines = TO_NUMBER(?) , C_Currency_ID = (?) , AD_User_ID = (?) , C_DocType_ID = (?) , SO_Res_Status = (?) , POReference = (?) , SalesRep_ID = (?) , Description = (?) , BillTo_ID = (?) , Delivery_Location_ID = (?) , EM_APRM_AddPayment = (?) , DocAction = (?) , CopyFrom = (?) , CopyFromPO = (?) , DeliveryViaRule = (?) , M_Shipper_ID = (?) , DeliveryRule = (?) , FreightCostRule = (?) , FreightAmt = TO_NUMBER(?) , IsDiscountPrinted = (?) , PriorityRule = (?) , C_Campaign_ID = (?) , ChargeAmt = TO_NUMBER(?) , C_Charge_ID = (?) , C_Activity_ID = (?) , AD_OrgTrx_ID = (?) , Quotation_ID = (?) , EM_Obpos_Applications_ID = (?) , EM_Obpos_Sendemail = (?) , EM_Obpos_Email_Status = (?) , EM_Obpos_Isdeleted = (?) , Calculate_Promotions = (?) , RM_AddOrphanLine = (?) , Convertquotation = (?) , C_Reject_Reason_ID = (?) , validuntil = TO_DATE(?) , EM_Obdisc_Addpack = (?) , C_Project_ID = (?) , C_Costcenter_ID = (?) , A_Asset_ID = (?) , User1_ID = (?) , User2_ID = (?) , EM_Obpos_Createdabsolute = TO_TIMESTAMP(?, ?) , PaymentRule = (?) , Posted = (?) , DateAcct = TO_DATE(?) , IsTaxIncluded = (?) , IsPrinted = (?) , IsInvoiced = (?) , IsDelivered = (?) , IsSelected = (?) , DropShip_User_ID = (?) , DropShip_BPartner_ID = (?) , DropShip_Location_ID = (?) , IsActive = (?) , AD_Client_ID = (?) , C_Order_ID = (?) , Deliverynotes = (?) , IsSelfService = (?) , C_Incoterms_ID = (?) , Incotermsdescription = (?) , Generatetemplate = (?) , Processing = (?) , Processed = (?) , DatePrinted = TO_DATE(?) , IsSOTrx = (?) , updated = now(), updatedby = ? " +
      "        WHERE C_Order.C_Order_ID = ? " +
      "        AND C_Order.AD_Client_ID IN (";
    strSql = strSql + ((adUserClient==null || adUserClient.equals(""))?"":adUserClient);
    strSql = strSql + 
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        INSERT INTO C_Order " +
      "        (AD_Org_ID, C_DocTypeTarget_ID, C_Return_Reason_ID, DocumentNo, DateOrdered, C_BPartner_ID, C_BPartner_Location_ID, RM_PickFromShipment, M_PriceList_ID, RM_ReceiveMaterials, DatePromised, RM_CreateInvoice, FIN_Paymentmethod_ID, C_PaymentTerm_ID, M_Warehouse_ID, InvoiceRule, DocStatus, GrandTotal, TotalLines, C_Currency_ID, AD_User_ID, C_DocType_ID, SO_Res_Status, POReference, SalesRep_ID, Description, BillTo_ID, Delivery_Location_ID, EM_APRM_AddPayment, DocAction, CopyFrom, CopyFromPO, DeliveryViaRule, M_Shipper_ID, DeliveryRule, FreightCostRule, FreightAmt, IsDiscountPrinted, PriorityRule, C_Campaign_ID, ChargeAmt, C_Charge_ID, C_Activity_ID, AD_OrgTrx_ID, Quotation_ID, EM_Obpos_Applications_ID, EM_Obpos_Sendemail, EM_Obpos_Email_Status, EM_Obpos_Isdeleted, Calculate_Promotions, RM_AddOrphanLine, Convertquotation, C_Reject_Reason_ID, validuntil, EM_Obdisc_Addpack, C_Project_ID, C_Costcenter_ID, A_Asset_ID, User1_ID, User2_ID, EM_Obpos_Createdabsolute, PaymentRule, Posted, DateAcct, IsTaxIncluded, IsPrinted, IsInvoiced, IsDelivered, IsSelected, DropShip_User_ID, DropShip_BPartner_ID, DropShip_Location_ID, IsActive, AD_Client_ID, C_Order_ID, Deliverynotes, IsSelfService, C_Incoterms_ID, Incotermsdescription, Generatetemplate, Processing, Processed, DatePrinted, IsSOTrx, created, createdby, updated, updatedBy)" +
