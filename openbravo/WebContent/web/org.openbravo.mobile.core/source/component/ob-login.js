...
enyo.kind({
  ...
  initComponents: function () {
    ...
    this.$.appNameVersion.setContent(OB.UTIL.VersionManagement.buildVersionString(OB.UTIL.VersionManagement.current.mobileCore));
    if (OB.UTIL.Debug.isDebug()) {
      this.$.appNameVersion.setContent(this.$.appNameVersion.getContent() + '-inDev');
    }
  ...
  setUserImages: function (inSender, inResponse) {
    ...
    if (!inResponse.data) {
      OB.Dal.find(OB.Model.User, {}, function (users) {
        ...
        me.renderUserButtons(name, userId, userName, image, connected, target);
      }, function () {
        OB.error(arguments);
      });
      return true;
    ...
    this.renderUserButtons(name, userId, userName, image, connected, target);
  },

  renderUserButtons: function (name, userId, userName, image, connected, target) {
    var i;
    if (name.length === 0) {
  ...
  postRenderActions: function () {
    ...
    if (!OB.UTIL.isHTTPSAvailable()) {
      this.$.loginWarning.setContent(OB.I18N.getLabel('OBMOBC_HTTPSMissing'));
    } else if (browserInfo.isPreferred) {
      // If browser is a recommended version show the normal login.
      this.$.loginWarning.setContent('');
    } else if (browserInfo.isSupported) {
      // If browser not recommended but supported, show a warning message.
      this.$.loginWarning.setContent(OB.I18N.getLabel('OBMOBC_LoginBrowserNotRecommended'));
    } else {
      // If browser not supported, show an strong warning message.
      this.$.loginWarning.setContent(OB.I18N.getLabel('OBMOBC_LoginBrowserNotSupportedButAllowed'));
    }
