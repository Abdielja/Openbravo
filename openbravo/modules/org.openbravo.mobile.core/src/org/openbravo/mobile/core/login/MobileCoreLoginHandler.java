...
public class MobileCoreLoginHandler extends LoginHandler {
  ...
  @Inject
  private MobileStaticResourceComponent staticResourceComponent;
    ...
    try {
      ...
      if (originalResult.has("showMessage") && originalResult.getBoolean("showMessage")) {
        ...
        if (session != null
            ...
            || ActivationKey.getInstance().checkOPSLimitations(sessionId) == LicenseRestriction.POS_TERMINALS_EXCEEDED) {
      ...
      if (role != null) {
        ...
        jsonMsg.put(
            "authenticationToken",
            MobileAuthenticationKeyUtils.getEncryptedAuthenticationToken(OBContext.getOBContext()
                .getCurrentClient().getId(), OBContext.getOBContext().getCurrentOrganization()
                .getId(), userId, role.getId(),
  ...
  protected boolean hasMobileAccess(Role role, String appName) {
    ...
    String hql = "select 1 from " + FormAccess.ENTITY_NAME
        + " where role.id=:roleId and specialForm.id=:formId and active='Y'";
    OBContext.setAdminMode(false);
    try {
      Query hbQuery = OBDal.getInstance().getSession().createQuery(hql);
      hbQuery.setParameter("roleId", DalUtil.getId(role));
