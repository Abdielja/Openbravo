...
/*global OB, console, _, enyo, Uint8Array, $ */
...
(function () {
  ...
  function S4() {
    // note: the single | is on purpose it is a bitwise or operation
    /*jslint bitwise: true */
  ...
  function processConsoleLevel(logLevel, args) {
    try {
      ...
      if (saveInServer) {
        ...
        // cut the message
        serverMessage = (serverMessage.length < 1000) ? serverMessage : serverMessage.substring(0, 1000);
      ...
      if (OB.UTIL.Debug.isDebug() || showInConsole) {
        saveInServer = saveInServer && (OB.MobileApp.model.get('logClientStatus') === 'OK');
  ...
  OB.UTIL.checkSourceVersion = function (sourceVersion, forceReload) {
    if (!window.localStorage.getItem("SourceVersion_" + OB.MobileApp.model.get('appName'))) {
      window.localStorage.setItem("SourceVersion_" + OB.MobileApp.model.get('appName'), sourceVersion);
    } else {
      if (window.localStorage.getItem("SourceVersion_" + OB.MobileApp.model.get('appName')) !== sourceVersion) {
        if (forceReload) {
          window.localStorage.setItem("SourceVersion_" + OB.MobileApp.model.get('appName'), sourceVersion);
